...
use opencv::prelude::*;
use opencv::{core::Mat, core::Size_, imgproc::resize, imgproc::INTER_AREA};
use ort::{value::Value, Environment, SessionBuilder};
...
pub fn detect_objects_on_image(mat: Mat) -> Vec<(f32, f32, f32, f32, &'static str, f32)> {
    let (input, img_width, img_height) = prepare_input(mat);
...
fn prepare_input(mat: Mat) -> (Array<f32, IxDyn>, u32, u32) {
    let (img_width, img_height) = (mat.cols(), mat.rows());
